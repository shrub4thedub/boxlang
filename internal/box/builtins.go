...
var builtins = map[string]BuiltinFunc{
	...
	// Process verbs - pure implementations without os/exec
	"run":   builtinRun,
	"spawn": builtinSpawn,
	"wait":  builtinWait,
...
func builtinLen(args []Value, scope *Scope) Result {
	...
	// Also output the result for command substitution
	fmt.Print(lengthStr)
...
// Process management verbs - pure Go implementations (no os/exec)
...
func builtinRun(args []Value, scope *Scope) Result {
	...
	// Box's "pure helpers" philosophy means we cannot use os/exec
	// Instead, provide helpful error explaining this is intentional
	return Result{Error: &BoxError{
		Message: "run: external command execution not supported in pure Box implementation",
		Help:    "Box uses only internal built-in verbs. Use built-ins like 'copy', 'move', 'download' instead of external tools.",
	}}
}
...
func builtinSpawn(args []Value, scope *Scope) Result {
	...
	// Box's "pure helpers" philosophy means we cannot use os/exec
	// Spawn would require process management which violates the pure helpers rule
	return Result{Error: &BoxError{
		Message: "spawn: background process spawning not supported in pure Box implementation", 
		Help:    "Box uses only internal built-in verbs for process management.",
	}}
}
...
func builtinWait(args []Value, scope *Scope) Result {
	...
	// Since spawn is not implemented, wait cannot function
	return Result{Error: &BoxError{
		Message: "wait: process waiting not supported in pure Box implementation",
		Help:    "Box uses only internal built-in verbs. Process management requires external system calls.",
	}}
}